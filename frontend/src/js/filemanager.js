...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	...
	var includeAssets = function(callback) {
        ...
        if(config.viewer.image.lazyLoad) {
            primary.push('/libs/lazyload/dist/lazyload.js');
        }
        if(config.customScrollbar.enabled) {
            primary.push('/libs/custom-scrollbar-plugin/jquery.mCustomScrollbar.css');
            primary.push('/libs/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.js');
        }
	...
	var FmModel = function() {
		...
		var TreeModel = function() {
			...
			this.nodeRendered = function(elements, node) {
				// attach context menu
				$(elements[1] || elements[0] ).contextMenu({  // lwf
